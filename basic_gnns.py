...
class GATNet(nn.Module):
    ...
    def forward(self, data):
        ...
        x = F.dropout(x1, p=self.drop_prob, training=self.training)
        x = self.conv2(x, edge_index)
...
class GATDetector(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, heads=8, drop_edge=0, drop_prob=0, bias=True):
        super(GATDetector, self).__init__()
        self.drop_prob = drop_prob
        self.conv1 = GATConv(in_dim, hidden_dim, heads, dropout=drop_edge, bias=bias)
        self.conv2 = GATConv(hidden_dim*heads, out_dim + 1, heads, concat=False, dropout=drop_edge, bias=bias)
...
class GCNDetector(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, drop_prob=0, bias=True):
        super(GCNDetector, self).__init__()
        self.drop_prob = drop_prob
        self.conv1 = GCNConv(in_dim, hidden_dim, bias=bias)
        self.conv2 = GCNConv(hidden_dim, out_dim + 1, bias=bias)

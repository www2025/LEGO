...
def query_certainty(prob_nc, number, nodes_idx):
    n_entropy = get_entropy_score(prob_nc).detach()

    scores = (n_entropy-n_entropy.mean())/(n_entropy.std())

    nodes_idx = np.array(nodes_idx)
    indices = torch.topk(scores[nodes_idx], number, largest=False)[1]
    indices = list(indices.cpu().numpy())
    return nodes_idx[indices]


def query_uncertainty(prob_nc, number, nodes_idx):
    n_entropy = get_entropy_score(prob_nc).detach()

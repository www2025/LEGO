...
# from data_process import generate_masks, generate_masks2, generate_masks3, generate_masks4
...
def train(args):
    ...
    def train2(model, optimizer):
        ...
        weights[-1] = 0.1  #Cora: 0.1    Amazon-cs: 0.2
    ...
    for iter in range(iter_num + 1):
        ...
        embeds, prob_nc, test_acc, auroc_ENT, aupr_0_ENT, aupr_1_ENT, fprs_ENT = evaluate(model)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--ID_classes', default=[0, 1, 2, 3, 4, 5, 6, 7, 8], type=list)
    ...
    # After the for loop where metrics are aggregated
    mean_test_acc = torch.stack(test_acc_list).mean().item()
    var_test_acc = torch.stack(test_acc_list).var().item()

    mean_auroc_ENT = np.mean(auroc_ENT_list)
    var_auroc_ENT = np.var(auroc_ENT_list)

    mean_aupr_0_ENT = np.mean(aupr_0_ENT_list)
    var_aupr_0_ENT = np.var(aupr_0_ENT_list)

    mean_aupr_1_ENT = np.mean(aupr_1_ENT_list)
    var_aupr_1_ENT = np.var(aupr_1_ENT_list)

    mean_fprs_ENT = np.mean(fprs_ENT_list)
    var_fprs_ENT = np.var(fprs_ENT_list)

    mean_precision = np.mean(precision_list)
    var_precision = np.var(precision_list)

    # Print final results with mean and variance
    print('Final average and variance of the Results:')
    print(f'Test Accuracy: Mean = {mean_test_acc}, Variance = {var_test_acc}')
    print(f'AUROC (ENT): Mean = {mean_auroc_ENT}, Variance = {var_auroc_ENT}')
    print(f'AUPR_0 (ENT): Mean = {mean_aupr_0_ENT}, Variance = {var_aupr_0_ENT}')
    print(f'AUPR_1 (ENT): Mean = {mean_aupr_1_ENT}, Variance = {var_aupr_1_ENT}')
    print(f'FPR95 (ENT): Mean = {mean_fprs_ENT}, Variance = {var_fprs_ENT}')
    print(f'Selection Precision: Mean = {mean_precision}, Variance = {var_precision}')
